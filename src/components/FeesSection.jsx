...
import { FaSearch, FaPlus, FaDollarSign, FaCalendar, FaUser, FaReceipt, FaCheck, FaTimes, FaFilter, FaPrint, FaDownload, FaEye } from 'react-icons/fa';
...
const FeesSection = () => {
  ...
  const submitBulkGenerate = (data) => {
    // Get student IDs based on selection (all, specific class, or specific class+section)
    let studentIds = [];
    
    if (data.generateFor === 'all') {
      studentIds = filteredStudents.map(student => student.id);
    } else if (data.generateFor === 'class' && data.selectedClass) {
      if (data.selectedSection) {
        // Generate for specific class and section
        studentIds = filteredStudents
          .filter(student => student.class === data.selectedClass && student.section === data.selectedSection)
          .map(student => student.id);
      } else {
        // Generate for specific class only
        studentIds = filteredStudents
          .filter(student => student.class === data.selectedClass)
          .map(student => student.id);
      }
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            ...
            <form onSubmit={(e) => {
              ...
              // Access the current state values directly
              const currentOptions = bulkGenerateOptions;
              const data = {
                generateFor: currentOptions.generateFor,
                selectedClass: currentOptions.selectedClass,
                selectedSection: currentOptions.selectedSection,
              ...
              <div>
                ...
                <select
                  value={bulkGenerateOptions.generateFor}
                  onChange={(e) => {
                    setBulkGenerateOptions({
                      ...bulkGenerateOptions,
                      generateFor: e.target.value,
                      selectedClass: '',
                      selectedSection: ''
                    });
                  }}
              ...
              {/* Class Selection (shown when "Specific Class" is selected) */}
              {bulkGenerateOptions.generateFor === 'class' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Class</label>
                  <select
                    value={bulkGenerateOptions.selectedClass}
                    onChange={(e) => {
                      setBulkGenerateOptions({
                        ...bulkGenerateOptions,
                    ...
                    className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                    required={bulkGenerateOptions.generateFor === 'class'}
                  >
                    <option value="">Select Class</option>
                    {/* Get unique classes from filtered students */}
                    {[...new Set(filteredStudents.map(student => student.class))].map(cls => (
              ...
              {/* Section Selection (shown when a class is selected) */}
              {bulkGenerateOptions.generateFor === 'class' && bulkGenerateOptions.selectedClass && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Section (Optional)</label>
                  <select
                    value={bulkGenerateOptions.selectedSection}
                    onChange={(e) => {
                      setBulkGenerateOptions({
                        ...bulkGenerateOptions,
                  ...
                  >
                    ...
                    {[...new Set(
                      filteredStudents
                        .filter(student => student.class === bulkGenerateOptions.selectedClass)
